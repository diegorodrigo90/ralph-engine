...
- `re-core` and `re-cli` SHALL expose typed runtime capability, agent, check, provider, policy, and hook registration so new capabilities can be added through shared contracts instead of command-local branching.
