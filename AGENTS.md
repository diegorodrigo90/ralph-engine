...
59. ALL dependencies SHALL pass `cargo audit` (zero CRITICAL/HIGH CVEs), `cargo deny check licenses` (allowlisted licenses only), `cargo deny check sources` (no unknown registries), and `cargo deny check bans` (no wildcard deps). These checks run in CI and pre-push hooks. New dependencies SHALL be evaluated for maintenance status, download count, and security history before adding.
60. GitHub Actions SHALL be pinned to full 40-character commit SHAs with version comment (`uses: owner/action@SHA # vX.Y.Z`). NEVER use tag-only (`@v6`) or branch (`@main`) references. `step-security/harden-runner` SHALL be the first step in every release job. `GITHUB_TOKEN` permissions SHALL follow least-privilege.
...
65. Core SHALL follow Model B (Mediator): core orchestrates the FLOW between plugins but NEVER inspects, transforms, or understands plugin-owned data. Core calls plugin A to export, passes the opaque result to plugin B to import. Plugins own their FORMAT and LOGIC. Core owns DISPATCH and LIFECYCLE. Specifically: (a) core SHALL NOT hardcode plugin-specific identifiers, status strings, file paths, or data formats; (b) core SHALL NOT contain business logic that belongs in a workflow, agent, context, or routing plugin; (c) all plugin interaction SHALL go through `PluginRuntime` trait methods or auto-discovery functions in `catalog.rs`; (d) plugins SHALL NOT call other plugins directly — core mediates all cross-plugin communication.

67. Every code review on RE SHALL verify Model B compliance (Rule 65). The CR checklist SHALL flag: (a) hardcoded plugin IDs in core (`"official.*"` strings outside `plugins/`); (b) hardcoded tool names, agent names, or workflow steps in core; (c) business logic that belongs in a workflow/agent/routing plugin leaking into core or TUI; (d) core knowing about specific data formats (diff syntax, agent stream schemas) instead of receiving classification from plugins; (e) test fixtures that couple to specific plugin identifiers instead of using generic test IDs. Violations SHALL be fixed before merge — no exceptions.
68. Core naming SHALL use generic, domain-neutral terms. Core SHALL NEVER use terms that imply specific plugin behavior: (a) NEVER `workflow`, `sprint`, `story`, `epic` — use `pipeline`, `task`, `item`; (b) NEVER `test`, `build`, `deploy`, `CR` as hardcoded identifiers — use plugin-declared labels; (c) NEVER `claude`, `codex`, `gemini` — use `agent`; (d) NEVER `Read`, `Edit`, `Bash` as tool names in core — plugins classify their tools via trait methods; (e) function/struct/module names in core SHALL describe the generic mechanism, not the specific use case. Core types: `Gate`, `Pipeline`, `Feed`, `Block`, `Agent`. Plugin types: `QualityGate`, `WorkflowStep`, `AgentSession`.
69. Core types that accept plugin-classified data SHALL receive that classification via trait methods, config, or auto-discovery — NEVER via hardcoded match arms.
70. Core error messages SHALL be generic and actionable. WHEN a plugin is missing or misconfigured, the error SHALL say "no plugin provides capability X" — NEVER "install official.bmad". The error MAY suggest `ralph-engine plugins list` to discover available options.
71. Audit and quality skills SHALL validate Model B compliance as a first-class check: scan core/ for hardcoded plugin IDs, tool names, and business logic. Model B violations SHALL be reported as HIGH severity findings. WHEN core needs to map external data to internal types (e.g., tool-name → block-kind), the mapping SHALL be provided by the plugin at registration time, not hardcoded in core. The test for this: "If I remove all official plugins, does core still compile and run with zero errors?" If not, Model B is violated.
72. The TUI SHALL follow a consistent design system. ALL visual decisions SHALL go through the `Theme` trait with semantic color slots — NEVER hardcoded `Color::Blue` or raw hex values in render functions. Spacing SHALL use named constants (`BLOCK_PADDING`, `SEPARATOR_STYLE`, `INDENT_WIDTH`) — NEVER magic numbers. Block rendering SHALL be consistent: same padding, same border style per block kind, same truncation rules. Every render function SHALL reference the active theme, not inline styles. The design system SHALL be documented and tested (snapshot tests for layout, theme contrast checks).
73. The TUI feed SHALL be scrollable, focusable, and copyable. (a) Scroll: `tui-scrollview` with follow mode (auto-scroll on new content, disable on user scroll-up, re-enable on `G`/`End`). Visual indicator when scrolled up. Mouse wheel support via `crossterm::EnableMouseCapture`. (b) Focus: `j`/`k`/click to focus a block, highlighted border on focused block, `Enter` to expand/collapse. (c) Copy: `y` on focused block copies clean content to clipboard via `arboard` (native) with OSC 52 fallback (SSH). Flash confirmation. Content SHALL be stripped of ANSI codes and border chars before clipboard write.
