...
- `re-core` and `re-cli` SHALL expose typed runtime capability, template, agent, check, provider, policy, and hook registration so new capabilities can be added through shared contracts instead of command-local branching.
...
- `tools/create-ralph-engine/` SHALL own developer scaffolding. Runtime catalog surfaces SHALL NOT turn scaffolding into a generic runtime responsibility.
