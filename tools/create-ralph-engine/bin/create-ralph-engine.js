...
function printHelp() {
  stdout.write(`create-ralph-engine\n\n`);
  stdout.write(`Usage:\n`);
  stdout.write(`  create-ralph-engine plugin <name> [options]\n`);
  stdout.write(`  create-ralph-engine --name <name> --publisher <publisher> [options]\n\n`);
...
function renderREADME(scaffold) {
  const lines = [
    `# ${scaffold.id}`,
    "",
    `Generated by create-ralph-engine.`,
    "",
    "## Summary",
    "",
    `- Kind: \`${scaffold.kind}\``,
