...
- Expandir os contratos do runtime além de metadados tipados, escopos de ativação, registro de capability, lifecycle, fronteiras de carregamento, runtime hooks, registro de topologia, health status, issue reporting e action planning para uma orquestração de estado mais rica.
- Fechar a publicação em npm e Homebrew sobre o pipeline endurecido de release no modelo build-once promote-later.
...
- Avaliar promover ainda mais os catálogos compartilhados de runtime, para que scaffold/runtime/testes de plugins derivem mais verificações de uma única fonte canônica.
