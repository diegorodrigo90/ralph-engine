...
- Official runtime catalog assembly stays plugin-owned: the CLI derives templates, prompts, agents, policies, and MCP servers from plugin-provided bundles instead of matching on plugin identifiers in shared catalog code
- Template contributions now carry typed embedded asset contracts, so the CLI can expose the real files a template delivers instead of stopping at metadata-only listings
